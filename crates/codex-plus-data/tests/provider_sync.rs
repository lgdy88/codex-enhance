use codex_plus_data::{ProviderSyncStatus, run_provider_path_repair, run_provider_sync};
...
fn provider_sync_repairs_sqlite_when_rollout_provider_matches_and_normalizes_paths() {
    ...
    assert_eq!(result.sqlite_rows_updated, 1);
    let first: serde_json::Value = serde_json::from_str(
        fs::read_to_string(home.join("archived_sessions/rollout-current.jsonl"))
            .unwrap()
            .lines()
            .next()
            .unwrap(),
    ...
    .unwrap();
    assert_eq!(first["payload"]["cwd"], "C:\\workspace");
    let db = Connection::open(home.join("state_5.sqlite")).unwrap();
    let row: String = db
...
#[test]
fn provider_path_repair_normalizes_paths_without_provider_converge() {
    let tmp = tempdir().unwrap();
    let home = tmp.path().join(".codex");
    fs::create_dir(&home).unwrap();
    fs::write(home.join("config.toml"), "model_provider = \"apigather\"\n").unwrap();
    let rollout = home.join("sessions/rollout-path.jsonl");
    write_rollout(&rollout, "openai", "thread-1", "\\\\?\\C:\\workspace");
    create_state_db(&home.join("state_5.sqlite"));

    let result = run_provider_path_repair(Some(&home));
