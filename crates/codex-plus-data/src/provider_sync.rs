...
pub fn run_provider_sync(codex_home: Option<&Path>) -> ProviderSyncResult {
    ...
    let sync_result = (|| -> anyhow::Result<ProviderSyncResult> {
        let changes = collect_session_changes(&home, Some(&target_provider))?;
...
pub fn run_provider_path_repair(codex_home: Option<&Path>) -> ProviderSyncResult {
    let home = codex_home
        .map(Path::to_path_buf)
        .unwrap_or_else(|| dirs_home().join(".codex"));
    if !home.exists() {
    ...
    let repair_result = (|| -> anyhow::Result<ProviderSyncResult> {
        let changes = collect_session_changes(&home, None)?;
        let rewrite_changes = changes
            .iter()
            .filter(|change| change.rewrite_needed)
            .cloned()
        ...
        let cwd_by_thread_id = changes
            ...
            .filter_map(|change| Some((change.thread_id.clone()?, change.cwd.clone()?)))
            .collect::<HashMap<_, _>>();
        let sqlite_update_count =
            count_sqlite_path_updates(&home.join("state_5.sqlite"), &cwd_by_thread_id)?;
        let global_state_update_count =
            count_global_state_updates(&home.join(".codex-global-state.json"))?;
        if rewrite_changes.is_empty() && sqlite_update_count == 0 && global_state_update_count == 0
        {
            return Ok(result(
                ProviderSyncStatus::Synced,
                "Provider path repair already up to date",
                &target_provider,
                None,
                0,
                0,
        ...
        let apply_result = (|| -> anyhow::Result<usize> {
            let sqlite_rows_updated =
                apply_sqlite_path_update(&home.join("state_5.sqlite"), &cwd_by_thread_id)?;
            apply_global_state_update(&home.join(".codex-global-state.json"))?;
            prune_backups(&home)?;
            Ok(sqlite_rows_updated)
        })();
    ...
    let _ = release_lock(&lock_dir);
    repair_result.unwrap_or_else(|err| {
        result(
            ProviderSyncStatus::Skipped,
            format!("Provider path repair skipped: {err}"),
            &target_provider,
            None,
            0,
            0,
...
fn collect_session_changes(
    ...
    for path in rollout_files(home)? {
        ...
        let provider_rewrite_needed = target_provider.is_some_and(|target| {
            payload.get("model_provider").and_then(Value::as_str) != Some(target)
        });
        let raw_cwd = payload.get("cwd").and_then(Value::as_str);
        let cwd_rewrite_needed = raw_cwd.is_some_and(|value| {
            cwd.as_deref()
                .is_some_and(|normalized| normalized != value.trim())
        });
        let rewrite_needed = provider_rewrite_needed || cwd_rewrite_needed;
...
fn to_desktop_workspace_path(value: &str) -> Option<String> {
    ...
    if stripped.starts_with(r"\\?\") {
        return Some(stripped[4..].replace('/', r"\"));
    }
    if is_windows_drive_path(stripped) || stripped.starts_with(r"\\") {
        return Some(stripped.replace('/', r"\"));
    }
    Some(stripped.to_string())
}

fn count_sqlite_path_updates(
    path: &Path,
