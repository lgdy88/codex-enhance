...
启用 `Provider 同步` 后，Codex++ 会在启动 Codex 前同步本地会话 metadata。它会把 rollout 文件和 SQLite 线程记录同步到当前 `model_provider`，并只对已有项目路径做格式归一化，让你切换供应商时不丢历史会话。
...
同步只修复会话可见性相关 metadata，不改写对话内容，不会把 rollout 里的项目路径强行覆盖到 SQLite，也不会批量新增 Desktop 项目根。如果 Codex 正在占用某个会话文件或 SQLite 忙碌，Codex++ 会跳过并继续启动，避免阻塞 Codex。
...
这个命令只会把 `\\?\D:\...` / `D:/...` 这类等价 Windows 路径修成 Desktop 更容易匹配的 `D:\...`，不切换 provider，也不移动会话所属项目。
