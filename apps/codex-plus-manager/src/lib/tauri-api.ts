...
function previewCommand(command: string, args?: Record<string, unknown>) {
  ...
  if (command === "check_update") {
    return { status: "ok", message: "Web 预览不检查更新。", currentVersion: "1.5.1", latestVersion: null, updateAvailable: false, progress: 0 };
  }
  ...
  if (command === "perform_update") {
    return { status: "failed", message: "Web 预览不安装更新。", currentVersion: "1.5.1", latestVersion: null, updateAvailable: false, progress: 0 };
  }
