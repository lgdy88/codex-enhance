...
fn release_workflow_builds_unified_desktop_installers() {
    ...
    assert!(workflow.contains("${{ runner.os }}-cargo-${{ hashFiles('Cargo.lock') }}"));
    assert!(workflow.contains("cache: npm"));
    ...
    assert!(workflow.contains("rm -rf desktop-dist"));
    assert!(workflow.contains("bundle/msi/Dex_${VERSION}_x64_*.msi"));
    assert!(!workflow.contains("bundle/msi/*.msi"));
    assert!(workflow.contains("Dex-${VERSION}-windows-x64.msi"));
    assert!(workflow.contains("Dex-${VERSION}-windows-x64.msi.sig"));
    assert!(workflow.contains("TAURI_SIGNING_PRIVATE_KEY"));
    assert!(workflow.contains("Missing TAURI_SIGNING_PRIVATE_KEY"));
    assert!(workflow.contains("tauri signer sign"));
    assert!(workflow.contains("const windowsName = `Dex-${version}-windows-x64.msi`;"));
    assert!(workflow.contains("Missing Windows updater asset"));
    assert!(workflow.contains("Missing signed Windows updater asset"));
    assert!(workflow.contains("platforms"));
    assert!(workflow.contains("\"windows-x86_64\""));
    assert!(workflow.contains("signature: windowsSignature"));
